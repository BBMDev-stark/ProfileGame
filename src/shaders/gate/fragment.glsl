...
// --- circle alpha mask ------------------------------------------------
// Portal opening from the GLB is a perfect circle (confirmed by XZ vertex
// analysis: r≈2.61, centred on the portal axis). uMaskEnabled=0 keeps old
// rectangular behaviour (World.jsx / World2.jsx).
uniform float uMaskEnabled;
uniform vec2  uMaskSize;   // width/height of the quad in local units (use equal x/y for circle)
uniform float uMaskEdge;   // softness of the mask edge (0.08 recommended for circle)
...
// Signed distance to a circle centred at origin.
// localPos: position relative to quad centre (0,0).
// size: quad width/height — radius = min(size.x, size.y) * 0.5
// so a square uMaskSize gives a perfect circle regardless of scale.
float circleMaskDist(vec2 localPos, vec2 size) {
...
void main() {
    ...
    // Circle mask — replaces the old round-top arch mask.
    // The portal opening is a perfect circle (XZ vertex plot, r≈2.61).
    // Pass equal x/y in uMaskSize (e.g. vec2(s, s)) for a true circle.
    if (uMaskEnabled > 0.5) {
        vec2 localPos = (vUv - 0.5) * uMaskSize;
        float d = circleMaskDist(localPos, uMaskSize);
        float mask = 1.0 - smoothstep(-uMaskEdge, uMaskEdge, d);
