...
export default function Html({ introFinished, setStartClicked, autoStart = false }) {
  ...
  useEffect(() => {
    ...
    if (autoStart) {
      // Coming from the portal-gate click: skip the START button entirely
      // and hand straight into the same "HI" -> scroll experience.
      gsap.set(".starting", { opacity: 0 });
      if (startingRef.current) startingRef.current.style.display = "none";

      timeline
        .to(".main", { opacity: 1, duration: 0.5, ease: "power3.inOut" })
        .call(() => handleStartClick());
    } else {
      timeline
        .to(".main", { opacity: 1, duration: 0.5, ease: "power3.inOut" })
        .to(".starting", {
  ...
  }, [introFinished, initializeAnimations, autoStart, handleStartClick]);
    ...
    <div className="main" ref={mainRef}>
      ...
      <div className="pref-1" ref={prefRef}>
        ...
        <div className="me-texts">
          <h1 className="me-text">BUI</h1>
          <h1 className="me-text">BINH MINH</h1>
        </div>
        <div className="title-container">
          <p className="left-text">Game Developer | Unity & C# | Interactive Tech</p>
        </div>
      </div>
